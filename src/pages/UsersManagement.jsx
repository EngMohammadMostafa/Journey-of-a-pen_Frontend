...
const UsersManagement = () => {
  const { user } = useAuth() // 🔐 جلب بيانات المستخدم المسجل
  ...
  // جلب بيانات المستخدمين من الباكند
  const fetchUsers = async () => {
    try {
      setLoading(true);
      setError(null);
      
      console.log('جاري جلب بيانات المستخدمين من الباكند...');
      ...
      if (response && response.data) {
        console.log('تم جلب البيانات من الباكند:', response.data);
        setUsers(response.data);
      } else {
        // إذا الباكند رجع بيانات فارغة
        console.log('الباكند رجع بيانات فارغة');
        setUsers([]);
      }
    } catch (error) {
      console.error('خطأ في جلب المستخدمين من الباكند:', error);
      setError('فشل في جلب بيانات المستخدمين من الباكند');
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    try {
      console.log('📤 إرسال طلب إضافة مستخدم للباكند:', formData);
      
      // إرسال طلب الإضافة للباكند
      await createUser(formData);
      
      // جلب البيانات المحدثة من الباكند
      await fetchUsers();
      
      setIsModalOpen(false);
      resetForm();
      alert('تم إضافة المستخدم بنجاح عبر الباكند');
      
    } catch (error) {
      console.error('❌ فشل في إضافة المستخدم:', error);
      alert('فشل في إضافة المستخدم - تأكد من اتصال الباكند');
    }
  };
  ...
  const handleUpdateUser = async (e) => {
    e.preventDefault();
    try {
      console.log('📤 إرسال طلب تعديل مستخدم للباكند:', {
        id: editingUser.id,
        data: formData
      });
      
      // إرسال طلب التعديل للباكند
      ...
      resetForm();
      setEditingUser(null);
      alert('تم تعديل المستخدم بنجاح عبر الباكند');
      
    } catch (error) {
      console.error('❌ فشل في تعديل المستخدم:', error);
      alert('فشل في تعديل المستخدم - تأكد من اتصال الباكند');
    }
  };
  ...
  const handleDeleteUser = async (userId) => {
    ...
    if (window.confirm('هل أنت متأكد من حذف هذا المستخدم؟')) {
      try {
        console.log('📤 إرسال طلب حذف مستخدم للباكند:', userId);
        
        // إرسال طلب الحذف للباكند
        await deleteUser(userId);
        
        // جلب البيانات المحدثة من الباكند
        await fetchUsers();
        
        alert('تم حذف المستخدم بنجاح عبر الباكند');
        
      } catch (error) {
        console.error('❌ فشل في حذف المستخدم:', error);
        alert('فشل في حذف المستخدم - تأكد من اتصال الباكند');
      }
    }
  };
  ...
  // أعمدة الجدول - متوافقة مع الباكند
  const columns = [
    { key: 'id', label: 'ID' },
    ...
    { key: 'gender', label: 'الجنس',
      render: (value) => {
        // توقع من الباكند: 'male'/'female' أو 1/2
        if (value === 1 || value === 'male') return 'ذكر';
        if (value === 2 || value === 'female') return 'أنثى';
        return value || '-';
      }
    },
    {  key: 'user_type', label: 'نوع المستخدم',
      render: (value) => {
        // توقع من الباكند: 1, 2,
        switch(value) {
          case 1: return 'مستخدم عادي';
          case 2: return 'أدمن';
      ...
      render: (_, user) => (
        <div className="actions-buttons">
          <button 
            ...
            disabled={!isAdmin} // 🔐 تعطيل الزر إذا لم يكن أدمن
            title={!isAdmin ? "ليس لديك صلاحية للتعديل" : "تعديل المستخدم"}
          >
            ✏️ تعديل
          </button>
          <button 
            ...
            disabled={!isAdmin} // 🔐 تعطيل الزر إذا لم يكن أدمن
            title={!isAdmin ? "ليس لديك صلاحية للحذف" : "حذف المستخدم"}
          >
            🗑️ حذف
          </button>
        </div>
  ...
  if (loading) {
    return (
      <div className="loading-container">
        <LoadingSpinner />
        <p>جاري جلب بيانات المستخدمين من الباكند...</p>
      </div>
    );
  }
    ...
    <div className="users-management">
      ...
      {!isAdmin && (
        <div className="admin-warning">
          ⚠️ <strong>تنبيه:</strong> أنت تستعرض بيانات المستخدمين فقط. 
          تحتاج إلى صلاحية الأدمن لإضافة/تعديل/حذف المستخدمين.
        </div>
      )}
      ...
      <DataTable 
        columns={columns}
        data={filteredUsers}
        emptyMessage="لا توجد مستخدمين لعرضهم - تأكد من اتصال الباكند"
      />
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                >
                  ...
                  <option value="1">ذكر (رقم)</option>
                  <option value="2">أنثى (رقم)</option>
                </select>
              </div>
